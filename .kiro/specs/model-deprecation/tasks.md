...
- [ ] 1. DeprecationInfo 構造体と ModelDescriptor インターフェースの拡張
- [ ] 1.1 非推奨メタデータを保持する embeddable 構造体を定義する
...
- [ ] 1.2 ModelDescriptor インターフェースに非推奨メソッドを追加する
  - `ModelDescriptor` インターフェースに `Deprecated()` と `DeprecatedRemovedIn()` の2メソッドを追加する
  - コンパイルエラーが発生することを確認し、次タスクで全モデルに対応する
  - _Requirements: 1.1, 1.2_

- [ ] 2. 全プロバイダーのモデル構造体に DeprecationInfo を embed する
- [ ] 2.1 (P) Anthropic プロバイダーのモデルに非推奨メタデータを適用する
...
- [ ] 2.2 (P) OpenAI プロバイダーのモデルに非推奨メタデータを適用する
...
- [ ] 2.3 (P) Groq プロバイダーのモデルに非推奨メタデータ構造体を embed する
  - 全 Groq モデル構造体に非推奨メタデータ構造体を embed する（全モデル非推奨でないためゼロ値）
  - _Requirements: 2.3_

- [ ] 2.4 (P) Cerebras プロバイダーのモデルに非推奨メタデータ構造体を embed する
  - 全 Cerebras モデル構造体に非推奨メタデータ構造体を embed する（全モデル非推奨でないためゼロ値）
  - _Requirements: 2.3_

- [ ] 3. models list コマンドに非推奨フィルタリングを追加する
- [ ] 3.1 一覧表示のビューモデルに非推奨情報を追加し、`--all` フラグでフィルタリングを制御する
...
- [ ] 4. models describe コマンドに非推奨情報表示を追加する
- [ ] 4.1 モデル詳細表示に非推奨ステータスと削除予定バージョンを追加する
...
- [ ] 5. 非推奨モデル使用時の警告を追加する
- [ ] 5.1 モデル解決時に非推奨モデルへの警告メッセージを stderr に出力する
  - モデル解決関数内で、解決されたモデルが非推奨の場合に stderr へ警告メッセージを出力する
  - 警告メッセージにはモデル名と削除予定バージョンを含める
  - 警告出力後も処理を中断せず、モデルの利用を継続する
  - _Requirements: 5.1, 5.2, 5.3_

- [ ] 6. Claude Code スキルの命名規則統一とリネーム
- [ ] 6.1 (P) `add-model` スキルを `model-add` にリネームする
...
- [ ] 6.2 (P) CLAUDE.md 等の参照箇所を更新する
  - プロジェクト内で `add-model` を参照している箇所を検索し、`model-add` に置き換える
  - _Requirements: 6.3_

- [ ] 7. model-deprecate スキルを作成する
- [ ] 7.1 `/model-deprecate` スキルの SKILL.md を作成する
